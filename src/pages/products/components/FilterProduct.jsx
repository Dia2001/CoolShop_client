      ...
      >
        ...
        <div className="flex w-[80%] m-1x flex-wrap gap-1">
            {materials.map((item,index)=>(
              <button className="bg-Black10 hover:bg-Black25 px-1x py-1 rounded-md focus:bg-Black75 focus:text-white italic"><p>{item}</p></button>
            ))}
        </div>
      </div>
      <div
        className={`relative w-full min-h-[120px] ${
          isToggle3 && "min-h-[40px] h-[40px]"
        } rounded-md border border-Black75 m-1 overflow-y-hidden transition-all duration-150 ease-in-out`}
      >
        <div className="flex justify-between">
