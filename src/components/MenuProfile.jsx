...
const MenuProfile = ({isShowProfile,setIsShowProfile}) => {
  ...
  return (
    <div>
      <div onClick={setIsShowProfile} className={`${isShowProfile &&'hidden'} fixed w-full h-full bg-BlackCool/25 z-30 transition`}></div>
    <div className={`${isShowProfile&&'translate-x-[100%]'} transition fixed right-0 top-0  backdrop-blur-md bg-white/90 rounded-l-[27px] h-[100vh] w-[450px] shadow-md z-30`}>
      ...
      <div className="flex flex-col gap-8 items-center">
        <button className="w-fit px-4x py-2x hover:opacity-75 bg-Primary text-white font-semibold shadow-sm">
          <h6>Chỉnh sửa thông tin</h6>
        </button>
        <button className="w-fit px-4x py-2x hover:opacity-75 bg-WarningColor text-white font-semibold shadow-sm">
          <h6>lịch sử đơn hàng</h6>
        </button>
        <button className="w-fit px-4x py-2x hover:opacity-75 bg-ErrorColor text-white font-semibold shadow-sm">
          <h6>Đăng xuất</h6>
        </button>
      </div>
    </div>
